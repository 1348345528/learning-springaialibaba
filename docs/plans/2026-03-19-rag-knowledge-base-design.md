...
**数据流说明：**
1. 文档上传 → 文档处理服务解析、拆分
2. chunk 保存至 MySQL（持久化）
3. **同步调用 RAG 检索服务 → 生成向量并存储至 Milvus**
4. Chat 问答 → RAG 检索服务查询 Milvus → 返回结果

### 2.3 服务职责

| 服务 | 端口 | 职责 |
|------|------|------|
| 文档处理服务 | 8081 | 文档上传、格式解析、文本拆分、知识块 CRUD、**同步索引到 Milvus** |
...
| 方法 | 端点 | 说明 |
|------|------|------|
| POST | `/api/vector/index` | **索引知识块到 Milvus**（由文档处理服务调用） |
